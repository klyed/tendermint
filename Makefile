...
HTTPS_GIT := https://github.com/klyed/tendermint.git
...
draw_deps:
	...
	@goviz -i github.com/klyed/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/klyed/tendermint
.PHONY: format
